...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    ordered_items_names = Item.select(:name).joins(:order_items).order(:name).distinct.pluck(:name)
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    # SELECT users.name, COUNT(order_items.id)
    # FROM users
    ...
    custom_results = User.select('name, COUNT(order_items.id) AS total_item_count').joins(:order_items).order(:name).group(:id)
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    # Note: during ruby group, we found that we needed FLOOR() because Postgres otherwise returned a numeric type with multiple decimal places.
    data = User
  ...
  it '30. returns the names of items that have been ordered without n+1 queries' do
    ...
    # ------------------------------------------------------
    orders = Order.includes(:items).all # Edit only this line
    # ------------------------------------------------------
